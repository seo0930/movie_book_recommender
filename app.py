...
# 카드 UI 함수
# =============================
def card(title, description, poster_url=None):
    st.markdown(
        f"""
        <div class="card">
            <h4>{title}</h4>
    ...
    if poster_url:
        st.image(poster_url, width=250, caption="영화 포스터")
...
movie_info = {
    ...
    "컨저링": "악령과 싸우는 실존 부부 엑소시스트의 공포 실화.",
    "겟 아웃": "남자친구가 여자친구 가족을 만나러 가며 벌어지는 미스터리 공포.",
    "곡성": "작은 마을에서 벌어지는 의문의 사건과 그 배후를 추적하는 이야기.",
    "해리포터": "마법 세계에서 성장하는 소년 해리와 친구들의 모험 이야기.",
    "반지의 제왕": "절대반지를 파괴하기 위한 여정에 나선 판타지 대서사.",
    "나니아 연대기": "옷장 속 판타지 세계를 여행하는 아이들의 모험 이야기."
}

book_info = {
    "하마터면 열심히 살 뻔했다": "일과 삶에 지친 사람들에게 위로와 공감을 주는 에세이.",
    "죽고 싶지만 떡볶이는 먹고 싶어": "우울과 일상을 기록한 심리 에세이.",
    "아주 작은 습관의 힘": "매일 1%씩 성장하는 습관의 과학을 다룬 책.",
    "부의 추월차선": "부자가 되는 관점을 완전히 바꿔주는 경제 자기계발 책.",
    "데미안": "내면의 나를 찾아가는 청소년의 성장 이야기.",
    "해리 포터": "마법사 소년의 모험과 성장 이야기.",
    "1984": "전체주의 감시사회를 비판한 디스토피아 소설.",
    "욱하는 마음 다스리기": "감정 조절과 스트레스 해소 방법을 알려주는 심리학 책.",
    "미움받을 용기": "타인의 시선에서 벗어나 자신의 삶을 살도록 돕는 철학적 대화."
}
...
mood_bonus = {
    ...
    "몰입감 있는 게 좋아요": "빠져드는 작품을 추천드릴게요!",
    "웃고 싶어요": "가볍고 유쾌한 작품이 잘 맞아요!",
}
...
book_moods = {
    "차분하게 읽고 싶어요": "잔잔하고 감성적인 책이 좋아요.",
    "동기부여 받고 싶어요": "불타오르는 동기부여를 드릴게요!",
    "몰입감 있는 스토리가 좋아요": "빠져드는 스토리 중심 책을 추천해요!",
}
...
if menu == "영화 추천":
    st.header("🎬 영화 추천")
    st.write("아래 질문에 답하면 취향에 맞는 영화를 추천해드릴게요!")

    genre = st.radio("어떤 장르가 좋아요?", list(movies.keys()))
    mood = st.radio("오늘 기분은 어때요?", list(mood_bonus.keys()))
    length = st.radio("선호하는 영화 길이는?", 
                      ["짧은 영화(110분 이하)", "보통", "긴 영화(140분 이상)"])
    ...
    if st.button("🎞 영화 추천 받기"):
        ...
        card("🎬 추천 영화", pick, poster_url=poster)
...
# =============================
    ...
    st.header("📚 책 추천")
    st.write("질문에 답하면 오늘 당신에게 맞는 책을 추천해드릴게요!")
